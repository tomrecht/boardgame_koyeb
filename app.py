...
# app.py (with shared board and timeout mitigation)
...
# Reuse a single board to keep caches across moves
shared_board = Board()
...
def select_moves():
    try:
        ...
        # Reset shared board with the new game state
        shared_board.update_state(state)
        logger.debug("select_moves: board updated")
...
def debug_piece_blots():
    # unchanged – uses a temporary board to avoid interfering with shared state
    ...

@app.route('/evaluate_board', methods=['POST'])
def evaluate_board():
    # unchanged – uses a temporary board
    ...
